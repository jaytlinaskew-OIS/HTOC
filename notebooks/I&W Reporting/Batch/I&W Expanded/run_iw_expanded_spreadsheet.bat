...
REM This script executes the I&W Expanded Spreadsheet Generator Python script for automated scheduling
...
set "SCRIPT_PATH=%~dp0I&W_Document_expanded_spreadsheet.py"
...
set "LOG_FILE=%LOG_DIR%\iw_expanded_spreadsheet_%date:~-4,4%%date:~-10,2%%date:~-7,2%_%time:~0,2%%time:~3,2%%time:~6,2%.log"
...
"%PYTHON_EXE%" -m pip uninstall -y pandas openpyxl pytz 2>nul
...
"%PYTHON_EXE%" -m pip install --quiet --upgrade --trusted-host pypi.org --trusted-host files.pythonhosted.org --disable-pip-version-check --no-warn-script-location pandas openpyxl pytz requests urllib3 2>nul || (
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo [%date% %time%] I&W Expanded Spreadsheet Generator completed successfully >> "%LOG_FILE%"
    type "%LOG_DIR%\temp_output.log" >> "%LOG_FILE%"
) else (
    echo [%date% %time%] I&W Expanded Spreadsheet Generator failed with error code %ERRORLEVEL%
    echo [%date% %time%] I&W Expanded Spreadsheet Generator failed with error code %ERRORLEVEL% >> "%LOG_FILE%"
    type "%LOG_DIR%\temp_output.log" >> "%LOG_FILE%"
)
