...
def fetch_attributes_data(indicators, ro):
    # Silence ThreatConnect SDK logger
    tc_logger = logging.getLogger('threatconnect')
    tc_logger.setLevel(logging.CRITICAL + 1)
    for h in tc_logger.handlers[:]:
        tc_logger.removeHandler(h)
    ...
    for indicator in indicators:
        try:
            ...
            if response.headers.get('content-type', '').startswith('application/json'):
                ...
                # If TC says ambiguous match, still return a placeholder so the indicator isn't dropped.
                if json_data.get('status') == 'Error' and json_data.get('errCode') == '0x1001':
                    attributes_data.append({
                        'id': None,
                        'summary': indicator,          # fall back to requested indicator
                        'type': None,
                        'ownerName': None,
                        'has_attributes': False,
                        'attribute_count': 0,
                        'createdBy.lastName': None
                    })
                    continue

                data = json_data.get('data', {}) or {}
                attrs = (data.get('attributes', {}) or {}).get('data', []) or []

                base = {
                    'id': data.get('id'),
                    'summary': data.get('summary') or indicator,
                    'type': data.get('type'),
                ...
                if attrs:
                    n = len(attrs)
                    for attr in attrs:
                        # normalize createdBy into a flat key used later
                        created_by_ln = None
                        cb = attr.get('createdBy')
                        if isinstance(cb, dict):
                            created_by_ln = cb.get('lastName')
                        else:
                            created_by_ln = attr.get('createdBy.lastName')
                        ...
                        row = {
                            ...
                            **base,
                            'has_attributes': True,
                            'attribute_count': n,
                            'createdBy.lastName': created_by_ln
                        }
                        attributes_data.append(row)
                else:
                    # No attributes -> emit a placeholder row
                    attributes_data.append({
                        **base,
                        'has_attributes': False,
                        'attribute_count': 0,
            ...
            # On any error, still keep the indicator via placeholder
            attributes_data.append({
                'id': None,
                'summary': indicator,
                'type': None,
