...
def main():
    ...
    print(f"[DEBUG] filtered_processed_data shape: {filtered_processed_data.shape}")
    ...
    # Debug: print shape of data_to_use
    print(f"[DEBUG] data_to_use shape: {data_to_use.shape}")
    if data_to_use is None:
        print("[WARNING] No filtered data available, using raw processed data.")
    ...
    if data_to_use is not None:
        try:
            ...
            if str(e) == "'search_term'":
                print("No indicators met all conditions as of this moment. No report generated.")
                sys.exit(2)
            else:
