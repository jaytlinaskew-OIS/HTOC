r"""
ThreatAssessScoringV5Daily — scheduled daily runner.
...
Exit contract for the .bat launcher:
  ...
  - print PIPELINE_OK_NOWORK and exit 0 when there is legitimately nothing to score today
  - non-zero exit on any hard failure (API/config/IO). Do not exit 0 without a marker.
"""
...
# Reliable logging under Task Scheduler (cp1252 consoles)
try:
...
if not today_indicators:
    print("No indicators first-seen today — nothing to score.")
...
if normalized_data:
    ...
    print("No valid indicator data found from ThreatConnect for today's indicators.")
    observed_src = pd.DataFrame()

print(f"observed_src: {len(observed_src):,} rows x {len(observed_src.columns)} cols")
if observed_src.empty:
    print("FATAL: ThreatConnect returned no HTOC Org rows for today's first-seen indicators.")
    sys.exit(3)
...
if not os.path.exists(excel_path):
    print(f"FATAL: Expected Excel output missing after write: {excel_path}")
    sys.exit(4)
print("PIPELINE_OK")
sys.exit(0)
