...
## 9. Smoke Test (`test.ipynb`)

The notebook `test.ipynb` demonstrates:

1. `from HTOCThreatConnect import get_v3_threatconnect_data`
2. Running the helper and inspecting the resulting DataFrame.

**Verify after install or config change:**

1. Open `test.ipynb` in Jupyter or VS Code.
2. Select the kernel that has the package installed (same environment as `pip install`).
3. Run all cells.
4. Confirm a non-empty DataFrame when the instance has matching data for the chosen `lastObserved_date`; an empty DataFrame is valid if no rows match the TQL window.
...
2. Run smoke tests (`test.ipynb` or a short script calling both helpers).
...
| `ModuleNotFoundError: HTOCThreatConnect` | Package not installed in active kernel | `pip install -e .` from repo root or install wheel into that environment |
