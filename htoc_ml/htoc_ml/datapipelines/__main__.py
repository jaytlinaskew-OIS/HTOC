"""python -m htoc_ml.datapipelines <command>"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args or args[0] in {"-h", "--help"}:
        names = " | ".join(COMMANDS)
        print(f"usage: py -3.13 -m htoc_ml.datapipelines {{{names}}} [options]")
        return 0
    command = args[0]
    if command not in COMMANDS:
        ...
        return 2
    rest = args[1:]
    module = importlib.import_module(f"htoc_ml.datapipelines.{COMMANDS[command]}")
