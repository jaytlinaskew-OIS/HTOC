...
REM Regenerate: py -3.13 -m htoc.datapipelines make-launcher --name search-tags
REM ============================================================================

if not exist "C:\Temp" mkdir "C:\Temp" >nul 2>&1
echo [%date% %time%] bat started from: %~dp0 > "C:\Temp\search_tags_debug.txt" 2>&1
...
set "LOG_DIR=%~dp0logs\search_tags"
...
set "LOG_FILE=%LOG_DIR%\search_tags_%TS%.log"
set "TEMP_OUT=%LOG_DIR%\temp_output_%TS%.log"

call :log START   Search indicators by tags
call :log CONFIG  Python: %PYTHON_EXE%
call :log CONFIG  Module: -m htoc.datapipelines search-tags --interactive
...
set "PKGS=pandas openpyxl tabulate"
...
call :log RUN     Launching -m htoc.datapipelines search-tags --interactive...
"%PYTHON_EXE%" -u -m htoc.datapipelines search-tags --interactive > "%TEMP_OUT%" 2>&1
