...
def package_root() -> Path:
    """``htoc_ml/`` directory (contains ``pyproject.toml`` and ``src/``)."""
    here = Path(__file__).resolve()
    for parent in here.parents:
        if (parent / "pyproject.toml").is_file() and (parent / "src" / "htoc").is_dir():
            return parent
    raise FileNotFoundError("cannot find htoc_ml package root (pyproject.toml + src/htoc)")


LAUNCHERS_DIR = package_root() / "launchers"
DATAPIPELINES_BATS_DIR = package_root() / "src" / "htoc" / "datapipelines" / "bats"
...
class LauncherJob:
    ...
    hidden: bool = True
    # "launchers" -> htoc_ml/launchers; "datapipelines" -> src/htoc/datapipelines/bats
    group: str = "launchers"
...
JOBS: dict[str, LauncherJob] = {
    ...
    "noi-daily-reports": LauncherJob(
        ...
        python_args="-m htoc.noi.daily_reports",
        packages="pandas numpy scikit-learn openpyxl",
        allow_nowork=True,
    ),
    ...
    "threat-score-iw": LauncherJob(
        ...
        python_args="-m htoc.datapipelines.threat_score_iw",
    ...
    "search-tags": LauncherJob(
        ...
        python_args="-m htoc.datapipelines search-tags --interactive",
    ...
    "triage": LauncherJob(
        ...
        log_prefix="triage",
        python_args="-m htoc.datapipelines triage",
    ...
    "iw-listing": LauncherJob(
        ...
        python_args="-m htoc.datapipelines iw-listing",
...
def repo_root() -> Path | None:
    """Full HTOC git checkout (has ``notebooks/ensure_htoc_data_share.bat``), else None."""
    here = Path(__file__).resolve()
    for parent in here.parents:
...
def _pythonpath_block(job: LauncherJob) -> str:
    # Package lives under src/ (setuptools package-dir); launchers do not pip-install -e.
    pkg = "%HTOC_ML_ROOT%\\src"
    if job.need_threatconnect_path:
        return (
            'set "PYTHONPATH=%HTOC_SHARE_ROOT%\\Data_Analytics\\threatconnect;'
            f'{pkg};%PYTHONPATH%"\n'
        )
    return f'set "PYTHONPATH={pkg};%PYTHONPATH%"\n'
...
def default_out_dir(job: LauncherJob) -> Path:
    if job.group == "datapipelines":
        return DATAPIPELINES_BATS_DIR
    return LAUNCHERS_DIR


def ml_root_bat_expr(dest: Path | None = None) -> str:
    """Climb from ``dest`` to ``htoc_ml/`` using ``%~dp0..`` segments."""
    folder = (dest if dest is not None else LAUNCHERS_DIR).resolve()
...
def _regenerate_hint(job: LauncherJob) -> str:
    if job.key:
        return f"py -3.13 -m htoc.datapipelines make-launcher --name {job.key}"
    return f'py -3.13 -m htoc.datapipelines make-launcher --new-name {job.name} --python-args "{job.python_args}"'


def render_bat(job: LauncherJob, dest: Path | None = None) -> str:
    ml_root = ml_root_bat_expr(dest if dest is not None else default_out_dir(job))
    return f"""@echo off
...
def render_vbs(bat_name: str) -> str:
    return (
        ...
        "' Generated by htoc.datapipelines.make_launcher. Path is relative to this .vbs.\n"
        "Option Explicit\n"
...
def write_job(job: LauncherJob, dest: Path) -> list[Path]:
    ...
    written[0].write_text(render_bat(job, dest), encoding="utf-8", newline="\r\n")
    if job.hidden:
...
def snapshot_production(dest: Path | None = None) -> list[Path]:
    ...
    root = repo_root()
    if root is None:
        print("skip production snapshot: HTOC repo root not found (standalone CodeBase)")
        return []
...
def install_share_helpers(dest: Path | None = None) -> list[Path]:
    ...
    Prefers copies already under ``htoc_ml/launchers/`` (or datapipelines/bats).
    Falls back to ``notebooks/`` in a full HTOC checkout.
    """
    ...
    for name in ("ensure_htoc_data_share.bat", "run_loggedoff_share_probe.bat"):
        candidates = [LAUNCHERS_DIR / name, DATAPIPELINES_BATS_DIR / name]
        if root is not None:
            candidates.insert(0, root / "notebooks" / name)
        src = next((path for path in candidates if path.is_file()), None)
        if src is None:
            raise FileNotFoundError(
                f"{name} not found in htoc_ml/launchers/ or notebooks/ "
                "(run from a full HTOC checkout once to seed launchers/)"
            )
        target = out / name
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Write standardized .bat + hidden .vbs launchers. "
            "NOI/PRISM -> htoc_ml/launchers/; datapipelines -> src/htoc/datapipelines/bats/."
        )
    )
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Override destination (default: per-job launchers/ or datapipelines/bats/)",
    )
    parser.add_argument(
        "--snapshot",
        action="store_true",
    ...
    parser.add_argument("--python-args", default="", help='Arguments after python.exe, e.g. "-m htoc.noi"')
    ...
    if args.all:
        ...
        group = "datapipelines" if "datapipelines" in args.python_args else "launchers"
        jobs.append(
            LauncherJob(
                key="",
                name=stem,
    ...
    if jobs:
        if args.out:
            ...
            for job in jobs:
                dest = default_out_dir(job)
                written.extend(install_share_helpers(dest))
                (dest / "logs").mkdir(exist_ok=True)
                written.extend(write_job(job, dest))
    if args.snapshot or args.all:
        written.extend(snapshot_production(LAUNCHERS_DIR / "production_copies"))
    for path in written:
        print(f"wrote {path}")
    return 0
